...
export default function RE() {
  ...
  return (
    ...
    >
      <h1> Stopwatch Time: {formattedTime} </h1> <h1> Reverse Engineering </h1>{" "}
      <h2> https: //github.com/Parth12358/ReverseEngineer</h2>
      <h3>
        Search through the folders and files to find the suspicious files and
        piece the answer together.These files belong to an UNREAL Engine Project
        so all files except the malicious ones have a purpose in game
      ...
      {isCorrect !== null && (
        <div style={{ marginTop: "20px", fontSize: "18px" }}>
          {" "}
          {isCorrect
            ? "Correct! Well done." && navigate("/ImgTestGame")
            : "Incorrect, try again."}{" "}
        </div>
      )}{" "}
    </div>
